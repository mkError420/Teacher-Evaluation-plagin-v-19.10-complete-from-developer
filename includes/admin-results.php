...
function tes_results_page() {
    ...
    $query = "SELECT s.*, t.name as teacher_name, t.department FROM $surveys_table s LEFT JOIN {$wpdb->prefix}tes_teachers t ON s.teacher_id = t.id";
    if (!empty($search_term)) {
        $like = '%' . $wpdb->esc_like($search_term) . '%';
        $query .= $wpdb->prepare(" WHERE s.title LIKE %s OR t.name LIKE %s", $like, $like);
    }
    $query .= " ORDER BY s.id DESC";
    $surveys = $wpdb->get_results($query);

    if (!empty($search_term) && empty($surveys)) {
        echo '<script>alert("No survey available");</script>';
    }

    // Get all teachers for filter
    $teachers = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}tes_teachers ORDER BY name ASC");
    $selected_teacher = 0;
    if ($selected_survey) {
    ...
    <div class="wrap">
        ...
        <form method="get" style="margin-bottom: 15px;">
            ...
            <div style="position: relative; display: inline-block; width: 300px;">
                <input type="text" name="s" id="tes_survey_search_input" value="<?php echo esc_attr($search_term); ?>" placeholder="Search Survey Title or Teacher..." style="width: 100%;" autocomplete="off">
        ...
        <form method="get" style="margin-bottom:20px;">
            ...
            <select id="tes_dashboard_teacher_filter" style="margin-right: 5px;">
            ...
            <select name="survey_id" id="tes_dashboard_survey_select" required>
        ...
        if ($selected_survey):
            ...
            $survey_details = $wpdb->get_row($wpdb->prepare(
                "SELECT t.name, t.class_name, t.phase 
            ...
            $submissions = $wpdb->get_results(
                $wpdb->prepare("SELECT s.answers, s.student_name, s.comment, s.submission_date
            ...
            foreach ($submissions as $sub) {
                ...
                foreach ($answers as $q_id => $answer) {
                    ...
                    if (isset($score_map[$answer])) {
                        ...
                        // Fallback for other questions (e.g. fixed questions)
                    ...
                    <tbody>
                        <?php foreach ($student_averages as $name => $data): ?>
                            <tr>
                                ...
                                <td><?php echo number_format($data['avg'], 2); ?> / 5</td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <?php if (!empty($comments)): ?>
                ...
                <div style="margin-bottom: 10px;">
                    <input type="text" id="tes-comment-search" placeholder="Search by Student Name..." style="width: 100%; max-width: 300px; padding: 6px; border: 1px solid #ddd; border-radius: 4px;">
                </div>
                <table class="widefat striped" id="tes-comments-table">
                    <thead>
                        <tr>
                            <th>Student Name</th>
                            <th>Comment</th>
                ...
                <script>
                jQuery(document).ready(function($) {
                    $('#tes-comment-search').on('keyup', function() {
                        var value = $(this).val().toLowerCase();
                        $('#tes-comments-table tbody tr').filter(function() {
                            var studentName = $(this).find('td:first').text().toLowerCase();
                            $(this).toggle(studentName.indexOf(value) > -1)
                        });
                    });
                });
                </script>
        ...
        <?php else: ?>
            ...
            <div class="card" style="background: #fff; padding: 20px; border: 1px solid #ccd0d4; margin-top: 20px;">
                <h2 style="margin-top: 0;">All Surveys List</h2>
                <table class="widefat striped">
                    <thead>
                        <tr>
                            <th>Survey Title</th>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?php echo esc_html($survey->teacher_name); ?></td>
                                <td><?php echo esc_html($survey->department); ?></td>
                                <td><?php echo esc_html(date('F j, Y', strtotime($survey->created_at))); ?></td>
                                <td>
                                    <?php echo $total_score_count > 0 ? number_format($avg_score, 2) . ' / 5' : 'No Data'; ?>
                                </td>
                                <td>
                                    <a href="<?php echo admin_url('admin.php?page=tes-dashboard&survey_id=' . $survey->id); ?>" class="button button-primary">View Result</a>
                                </td>
                            </tr>
                        <?php endforeach; else: ?>
    ...
    jQuery(document).ready(function($) {
                    ...
                    success: function(response) {
                        ...
                        if (response.success && response.data.length > 0) {
                            ...
                            $.each(response.data, function(i, item) {
                                ...
                                html += '<div class="tes-survey-suggestion-item" data-id="' + item.id + '">' + escapedTitle + teacherInfo + '</div>';
                            });
                            suggestionsBox.html(html).show();
                        } else {
                            suggestionsBox.html('<div class="tes-survey-suggestion-item" style="cursor: default;">No survey available</div>').show();
                        }
                    }
                });
            }, 300);
        ...
        var $teacherSelect = $('#tes_dashboard_teacher_filter');
        var $surveySelect = $('#tes_dashboard_survey_select');
        ...
        function filterSurveys() {
            ...
            if (!teacherId) {
                $surveySelect.prop('disabled', true);
                $surveySelect.val('');
            } else {
                $surveySelect.prop('disabled', false);
                $surveyOptions.each(function() {
                    var $opt = $(this);
                    if ($opt.val() === '') return; // Always show default
        ...
        // Reset all fields automatically on page load
        searchInput.val('');
        $teacherSelect.val('');
        $surveySelect.val('');
